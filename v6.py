...
class Calculator:
    def __init__(self, master):
        ...
        self.log_label.grid(row=0, column=5, padx=5, pady=5)

        self.log_text = tk.Text(master, width=30, height=10, state=tk.DISABLED)
        self.log_text.grid(row=1, column=5, rowspan=4, padx=5, pady=5)

        self.scrollbar = ttk.Scrollbar(master, command=self.log_text.yview)
        self.scrollbar.grid(row=1, column=6, rowspan=4, sticky='ns')
    ...
    def _format_result(self, total_inches):
        ...
        return f"{feet} ft {inches}\"{closest_fraction}"
