...
class Calculator:
    def __init__(self, master):
        ...
        self.log_label.grid(row=6, column=0, padx=5, pady=5) # Moved to row 6

        self.log_text = tk.Text(master, width=45, height=10, state=tk.DISABLED)  # Increased width
        self.log_text.grid(row=7, column=0, rowspan=4, columnspan=4, padx=5, pady=5) # Moved to row 7

        self.scrollbar = ttk.Scrollbar(master, command=self.log_text.yview)
        self.scrollbar.grid(row=7, column=4, rowspan=4, sticky='ns') # Moved to row 7
