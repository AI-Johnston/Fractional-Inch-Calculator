...
class Calculator:
    def __init__(self, master):
        ...
        self.feet_entry = ttk.Entry(self.measurement1_frame, width=10)
        ...
        self.inches_entry = ttk.Entry(self.measurement1_frame, width=10)
        ...
        self.inches_entry2 = ttk.Entry(self.measurement2_frame, width=10)
        ...
        self.fraction_entry2 = ttk.Entry(self.measurement2_frame, width=10)
        ...
        self.log_label.grid(row=6, column=0, padx=5, pady=5)

        self.log_text = tk.Text(master, width=45, height=10, state=tk.DISABLED)
        self.log_text.grid(row=7, column=0, rowspan=4, columnspan=4, padx=5, pady=5)

        self.scrollbar = ttk.Scrollbar(master, command=self.log_text.yview)
        self.scrollbar.grid(row=7, column=4, rowspan=4, sticky='ns')
        self.log_text['yscrollcommand'] = self.scrollbar.set

        # Number Pad - Removed for clarity in this stage, will be added back later
    ...
    def calculate(self, operation):
        ...
        if value1 is not None and value2 is not None:
            try:
                ...
                result_formatted = self._format_result(total_inches)
                self.result_value.config(text=result_formatted)

                # Log the calculation
                log_entry = f"{self._format_input(value1)} {operator} {self._format_input(value2)} = {result_formatted}\n"
                self.log_calculation(log_entry)

            except Exception as e:
                self.result_value.config(text=f"Error: {e}")

    def _format_input(self, total_inches):
        """Helper function to format the input values into feet, inches, and fraction string."""
        feet = int(total_inches // 12)
        remaining_inches = total_inches % 12
        inches = int(remaining_inches // 1)  # Get the whole number of inches
        fractional_inches = remaining_inches - inches
